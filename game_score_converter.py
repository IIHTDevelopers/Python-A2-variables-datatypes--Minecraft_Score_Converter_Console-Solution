...
def convert_string_to_int(mining_score):
    ...
    if not isinstance(mining_score, str) or not mining_score or not mining_score.isdecimal():
...
def convert_float_to_int(combat_score):
    ...
    # Input validation - FIXED VERSION
    if not isinstance(combat_score, float) or combat_score < 0:
...
def convert_hex_to_int(achievement_hex):
    ...
    # Input validation - FIXED VERSION (handles empty string)
    if (not isinstance(achievement_hex, str) or
        not achievement_hex or  # Explicitly check for empty string
        not all(c in '0123456789ABCDEFabcdef' for c in achievement_hex)):
...
def convert_score_to_string(total_score):
    ...
    # Input validation - FIXED VERSION (exclude booleans explicitly)
...
def create_player_list(player_name, total_score):
    ...
    # Input validation - FIXED VERSION (check for whitespace-only strings)
